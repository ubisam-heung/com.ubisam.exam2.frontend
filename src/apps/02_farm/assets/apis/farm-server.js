import $base from "@/plugins/apis";
import $userinfo from "@@/assets/stores/userinfo.js";

const name = "[/assets/apis/farm-server.js]";
...
const $server = {
  ...
  api: {
    execute(optionsBuilder){
      return $base.meta
        .env("VITE_FARM_SERVER", "VITE_OAUTH2_SERVER")
        .then(optionsBuilder)
        .then((e) => {
    ...
    url(env, path){
      ...
      return `${env["VITE_FARM_SERVER"]}${paths}`;
    },
    token(env) {
